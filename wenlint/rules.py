...
RULES = [
    ...
    {
        "id": "C004", "name": "empty-escalation", "category": "空泛递进",
        "severity": "suggestion", "semantic": True,
        "message": "递进句式「{w}」——核对是否提供了实质新信息",
        "patterns": [r"(?:不仅|不只是|不单是|不止是)(?=[^。！？；\n]{1,80}?[，,][ \t]*(?:更是|更|而是|还是|也))"],
        "review_hint": "只判断递进是否有实质信息，不把句式本身判为错误。"
                       "'不仅是升级，更是革新'若只是重复评价，按上下文作最小改写；"
                       "'不仅支持导出，也支持导入'有新增能力，应 KEEP。"
                       "引述、真实范围区分和必要强调可 KEEP；不能为了具体化编造功能或数据，"
                       "可查证的断言用 VERIFY，缺少作者信息用 ASK。",
    },
    # ---------- H: 模糊词 ----------
    {
    ...
    # ---------- M: 成稿中的讨论史 / 旧上下文依赖 ----------
    {
        ...
        "patterns": ["讨论过程", "经过讨论", "经讨论", "讨论后", "讨论中",
                     "讨论结果", "讨论结论", "讨论了",
                     "纠偏说明", "纠偏", "纠正", "修正", "更正",
                     "改为", "改成", "恢复为", "原先", "此前", "先前"],
        "semantic": True,
    ...
    },
    {
        ...
        "patterns": ["不再", "仍然", "仍旧", "依然",
                     r"(?<![优领祖神抢占争率首原])先(?![生进验导端烈贤天决知驱兆例后祖前])",
                     r"(?<!不)再(?![现生版造保就审])"],
        "semantic": True,
    ...
    # ---------- E: 空洞强调 ----------
    {
        ...
        "review_hint": "判断强调是否有实际内容支撑：能给出具体程度/数据/"
                       ...
                       "口语/对话场景可放宽（KEEP）。具体描述必须来自正文或可追溯资料；"
                       "缺少依据时 VERIFY/ASK，不得补造数字、经历或引文。",
    },
    # ---------- R: 冗余结构 ----------
    {
