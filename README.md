...
- 默认使用 DeepSeek（`https://api.deepseek.com` / `deepseek-v4-flash`），也可更换兼容服务。API Key 只保存在当前进程内存中，不写配置文件或日志。
...
- ASK 显示为“待补充”：填写作者信息后只发送相关片段，生成一条待确认改写；不足时继续追问，失败和取消保留输入。长文先完成剩余段落，再补充改写。
...
- Agent 的计划、工具调用、工具结果和结论摘要实时可见，摘要随流式输出更新；生成中的预览不作为已校验结论或自动修改。支持取消，不记录模型内部思维链。
- 流式请求按连续无数据计时（默认90秒），数据和心跳到达会刷新空闲计时；短文审查保留5分钟总上限，长文保留每批120秒预算。连接拒绝、证书、DNS、超时和HTTP错误分别提示，可修改设置后重试。
- 参考资料按页读取（默认40行、最多4000字符），长单行有字符续页游标；搜索明确标注预算截断，不把未搜索区域误称为已查。
- “保存”经再次确认后覆盖原文件，“另存为”创建新文件，“应用到编辑区”只更新当前编辑内容。覆盖前会校验文件未被外部修改。

下载 [v0.6.2 桌面应用](https://github.com/DreamStars1/WenLint/releases/tag/v0.6.2)：提供 Windows x64、Apple 芯片 Mac 和 Intel Mac 三种 ZIP，附校验和与构建清单。开发者可在 Windows 上构建：
...
| C004 | 空泛递进 | candidate | `不仅…，更…` 等同一行/单元格配对句式；真实新增信息保留，不按句式判错 |
...
全文语义审查还会核对空泛评价、句间关系与无实质信息的递进，保留合理学术审慎及原稿文风。
桌面首轮审查中，改写增删、重排或改变阿拉伯数字文本（含常见日期、版本、正负号和百分号）时，
会转为“待补充”，不进入修改稿；作者补充后仍需采纳确认。格式转换也会触发。
这只是保守的文本保护，不验证事实，也不覆盖中文数词、单位和语义变化。
中文正反例及完整验收边界见 [写作质量审查](references/writing-quality.md)。
