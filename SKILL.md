...
全文审查或改写时读取 `references/writing-quality.md`：补充空泛评价、句间关系、空泛递进和文体判断，以及改写后的事实与语义保真验收。不要将句式、三项列举或标点次数本身判为错误。
...
- **C004** 空泛递进候选（`不仅…，更… / 不只是…，而是…`）：核对实质新信息，正常新增能力或范围区分 KEEP；只覆盖同一正文行或表格单元格中的配对句式
