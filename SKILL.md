...
2. **运行 wenlint**——`cd ~/文档/脚本/wenlint && python -m wenlint <path> --json`（venv python）
3. **四分类判断**——对每条 finding：先读 `review_hint`（规则已给判断方向），再看 `sentence/before/after` 理解语境：
   ...
   - `VERIFY` 可核实断言（时间/数量/能力/归因）→ 先搜：本地 search_files → 网络；找到依据 → REWRITE 附依据出处；找不到 → ASK
   - `ASK` 资料不足 → 列疑问请作者确认
4. **VERIFY 检索**——搜用户现有文件优先（需求文档/PRD/README），依据必须可追溯（文件:行/URL），禁止编造出处
5. **只改确认要改的局部**——每次处理一条 finding，先给 diff 预览；无依据不删不确定词（`可能/大概`）；用户确认后写盘。除非用户明确要"整体润色"，不做全文润色。
